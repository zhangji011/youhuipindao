  ...
  created() {
    this.pullDownInitTop = -50;
        Bus.$on('activee',() => {
          ...
          // console.log(this.scroll.refresh + '22222')
    })
    ...
    initScroll() {
      ...
      let options = {
        ...
        // directionLockThreshold:10
				...
				swipeTime:1500   ,//设置 momentum 默认2500
      }
      ...
      if (this.pullUpLoad) {
		...
		// 20191021新增 首页限时抢购,分享动画隐藏需要
		if(this.listenScrollStart){
			this.scroll.on("scrollStart", () => {
				this.$emit("listenscrollstart",false);
			});
		
		// 20191021新增 首页限时抢购,分享动画隐藏需要
		if(this.listenScrollEnd){
			this.scroll.on("scrollEnd", () => {
				this.$emit("listenscrolledn",true);
			});
		}}
    },
    disable() {
