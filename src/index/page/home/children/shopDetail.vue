...
export default {
  ...
  computed: {
	...
	destroyed() {  //20190920页面销毁时取消监听
	  window.removeEventListener('popstate', this.fun, false); //false阻止默认事件
	},
  mounted() {
		if (window.history && window.history.pushState) { //20190920判断浏览器是否支持popstate
  ...
  methods: {
    ...mapMutations([
       ...
       'ALERTTEXTFIRST'
		 ...
		 fun(){
			 // history.pushState(null, null, document.URL);  //解决第二次点击禁止
			 // alert("shopdetail页面监听到了安卓物理键返回！！！！！！！！！")
			 this.$router.go(-1);
		 },
    init() {
      ...
      if(isHebaoApp()) {
        ...
        axios.post("getShopInfoDetailTest", {  //测试接口
