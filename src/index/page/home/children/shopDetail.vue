  ...
  <div class="home">
    ...
    >
      <div class="content">
		...
		<!-- 20191017改优惠信息 -->
        <div class="nullHeight" v-if="islogin && (supportList >= 1 || couponList.length>=1)"></div>
        <section class="s_3" v-show = "islogin && couponList.length>=1">
        <!-- <section class="s_3"> -->
          <div class="title">
            <p>优惠信息</p>
            <div class="hr-2"></div>
		...
		<!-- 20191017改查看支持本店消费的和包劵 -->
		<section class="address" v-if="islogin && supportList >= 1" :class="{lineshow:couponList.length>=1}" style="padding:0 0.9375rem;">
			<!-- <div class="address-info" @click="goDetail($event,shopData,2)">  20191018注释,改成同步外放 -->
			<div class="address-info" @click="couponClick()">
			  <div class="left">
			    <img src="/static/img/voucher_icon.png"/>
			    <a>支持本店消费的和包券</a>
			  </div>
        ...
        <section class="s_4" v-show="islogin">
		  ...
		  <!-- 遍历满减 -->
          <!-- <div class="address" v-for="(item,index) in ceshi.data.rec" @click="jumpInfo(item)"> -->
		  ...
		  <!-- 遍历满送 -->
		  <!-- <div class="address" v-for="(item,index) in ceshi.data.drawRec" @click="jumpInfo(item)"> -->
		  <div class="address" v-for="(item,index) in shopData.drawRec" @click="jumpInfo(item)">
		    <div class="address-info">
		      <div class="left_left">
		        <span class="list-flg">满送</span>
		        <!-- <span class="activity">{{item.GME_NM}}：{{item.GME_ABBR}}</span> -->
		        <span class="activity">{{item.drawNm}}</span>
		      </div>
		      <div class="right">
		        <span class="link">查看规则</span>
		        <img class="special" src="/static/img/link_button.png"/>
  ...
  methods: {
    ...
    init() {
      ...
      let params = this.shopParm;
        axios.post("getShopInfoDetailTest", {  //测试环境调用接口   走文件
          ...
          merc_id: this.shopParm.merc_id,
            ...
            if (res.code==="0") {
			  ...
			  axios.post("getPrdinfo", {
			      merchantId: 888073118000170,  //商户编号
			      mobileNo: this.token.productNo,
			      // mobileNo: 15111427793,  //手机号
			      qryMercId: this.shopData.mercId,  //查询电子券的商户号
			      qryFlg: 1  //查询标识（1：查余额及总记录数：2：查明细）
			    }).then(res => {
			      this.supportList = res.data.curCnt;  //电子券的张数
			    })
            }
        });
    },
	couponClick() {
	  let qryMercId = this.shopData.mercId;
	  this.$router.push({
	    path: "/consume",
	    query: {
	      params: qryMercId
    ...
    getCoupon() {
      axios.post('queryCoupon',{
        "session": this.token.session.replace(/\+/g, "%2B")  //正式
        // "session": "J4nD91f1SmyvSLK18VEivyCwqAU2HLCoEQulXnoF/3A="  //测试
      }).then(res => {
          if(res.code === "0") {
