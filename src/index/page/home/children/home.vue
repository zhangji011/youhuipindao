...
export default {
  data() {
    return {
	  ...
	  // console.log("执行销毁某个方法——————————————————————————————")
	  // this.$destroy(HomeSlider);
activated(){
	Bus.$emit('couponScroll');  //解决有时候领劵专区卡顿
  ...
  methods: {
    ...
    getMainCoupon() {
      axios.post("queryCouponMainStation",{
        "session": this.token.session.replace(/\+/g, "%2B")   // 单点登录返回session
				// "session":"J4nD91f1SmyvSLK18VEivyCwqAU2HLCoEQulXnoF/3A="  //测试查看数据使用的session
      }).then(res => {
        if(res.code === "0") {
          let data = res.data;
