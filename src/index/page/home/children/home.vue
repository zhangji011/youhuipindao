<template>
  <div class="home" @touchstart="noscroll($event)">
		<!-- 客户端分享按钮 -->
		<transition name="slide-fade">
		<div v-show="isShareBtn && animatshow" @click="getSharePage()" class="shareBtn">
			<img src="../../../../../static/img/shareicon.png" alt="">
		</div>
		</transition>
		<transition name="slide-fade">
		<div v-show="isShareBtn && animatshow && nodata" @click="getSharePage1()" class="shareBtn1">
			<img src="../../../../../static/img/activeicon.png" alt="">
		</div>
		</transition>
    <!-- <div :class="isShow">
      <span>优惠</span>
    ...
    <!-- 弹窗样式1 只有一张图情景 -->
   <div v-if="groupp && alertinfo && alertinfo.length==1" :class="isAlertInfo" @touchend.stop>
	...
	<!-- 20191010加弹框改成轮播 多张情景 -->
		<alert-silder v-show="groupp" :bannerinfo="alertinfo" @bannerclick="goDetail"></alert-silder>
    ...
    <scroll
	  ...
	  @listenscrollstart="animationshow"
	  @listenscrolledn="animationshow"
    >
    <div :class="isShow2">
...
import alertSilder from "@@/components/base/alertSilder";   //20191010改造弹框
...
export default {
  data() {
    return {
      ...
      jdFlag: false,
      scrollbar: false,
	  animatshow:true ,//限时抢购和分享的显示隐藏
		groupp:true,  //秒杀京东商品详情返回
		nodata:false //限时抢购有无数据
    };
  },

  mounted() {
	  var that=this;
		if(Cookies.get('groupp')){
			Cookies.remove('groupp');
			if(this.groupp){
				this.groupp=false;
			}
		}
	...
	//20191024加  判断限时抢购是否有数据
	this.getFlashaleData();
  },
  activated(){
  Bus.$emit('couponScroll');  //解决有时候领劵专区
  Bus.$emit('scrollGoods');  //专题营销位
  ...
  methods: {
	...
	getFlashaleData(){//20191024加  判断限时抢购是否有数据
		axios.post("queryLimitedCommodities").then(res => {
			console.log(!res.data[0].limitedCommodities.length)
			  if(res.code=="0" && res.data && res.data[0].limitedCommodities.length>0){
				  this.nodata=true;
			  }
		  })
		  .catch(res => {
...
noscroll(e){
   if(e.target.className.indexOf("ZJgoods")!=-1){
    // e.cancelBubble = true;
    e.stopPropagation();
   }
  },
...
.shareBtn {
  width: 2.5rem;
  height: 2.6rem;
  // background: url("/static/img/shareicon.png") top no-repeat;
...
.shareBtn1 {
  width: 3rem;
  height: 3rem;
  // background: url("/static/img/activeicon.png") top no-repeat;
  // background-size: 100% 100%;
  position: fixed;
  top: 21.7rem;
  right: 0.3rem;
