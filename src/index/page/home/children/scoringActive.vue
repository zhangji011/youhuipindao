    			...
    			<ul class="bakimg">
    				<li>
    					...
    					<div class="activeIntroduce">
    						<div>
    							<p class="introduceP2">活动期间，和包支付指定湖南绑卡用户（含电信、联通用户）在活动页进行报名即可参与积分打榜计划活动，参与成功的用户在长沙线下商户使用和包支付扫码付消费，每前往1个线下商户消费成功即可获得1个积分，且单个商户仅限获得1次积分，已参与商户积分不重复累积（含连锁商户）。全员积分排名页面实时更新，用户参与活动获得的积分可在活动页面我的权益兑换成和包券消费使用（首月1积分=2元和包券，次月1积分=1元和包券，同一用户每月兑换额度500积分封顶）。</p>
    						</div>
    					</div>
    					<!-- 点我打榜赢好卷 -->
    					<div class="btnbox">
    					...
    					<div class="rule">
    						...
    						<p>3、本次活动积分兑换和包券时间截止到活动结束，用户兑换的和包券为通用券，领取后于2020年1月31日24点失效，该和包券不支持转赠转让、分享、不可叠加使用，详情见和包支付客户端单张和包券使用细则；</p>
...
<script>
  import {
	...
	export default {
        ...
        isshow:false  ,//默认不展示
		showrule:"false"  //传到top100页,是否自动弹框
			}
		},
		 computed: {
		  ...mapState([
      ...
      getSharePage(e) {
        ...
        let shareTxt = "打榜活动火热开启，湖南和包支付指定用户线下消费得积分，积分可兑换优惠，速来参与吧！";
        shareNow(index_urls.shareUrl, shareTxt);
      }
		},
		mounted() {
			this.showrule=this.getUrlKey("showrule")||"false"; //默认不展示弹框
      // this.isAttend="N";
