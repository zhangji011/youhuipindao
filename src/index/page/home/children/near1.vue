    ...
    <!-- <scroll
        ...
        @pullingUp="onPullingUp"> -->
          ...
          <ul v-for="(item,index) in shopList"  @click="goSeller(item)" :key="item.id">
            <!-- :key="item.TX_JRN" -->
              ...
              <li class="middle">
                  ...
                  <!-- <div v-show="true" class="c3" > -->
                      <span v-for="(item2,ide2) in item.DRAW_REC" class="b" :class="{noshow:ide2>2}" >{{item2.drawNm}}</span>
                      <span v-for="(item1,ide) in item.ACT_INF" class="b" :class="{noshow:(item.DRAW_REC.length+ide)>5}" >{{item1.GME_ABBR}}</span>
                  </div>
              </li>
              <li class="right">
                  <div class="c4">
          ...
          <ul v-if = "!shopList || shopList.length<=0 ">
            ...
            <li @click="aginEnter()" class="aa" v-if="!showLoading">
