<template>
  <div>
    ...
    <div class="home">
			<!-- 20191011加复制导航栏 -->
			<section style="position: fixed;left: 0;top:5.8rem;z-index: 9999;" v-show="slider1 && slider1.length>0 && istop0" class="s_3 s" :class="{suctionTop:true}">
			  <ul>
...
export default {
  ...
  methods: {
	...
	scrolltopY(pos) {  //20191012加 优惠导航栏吸顶事件
		if(pos.y<-(16 * (document.documentElement.clientWidth / 375) * 10.625)){
			if(this.istop0!=true){
				this.istop0=true;
			}
		}else{
			if(this.istop0!=false){
				this.istop0=false;
			}
		}
	},
    ...
    goCatalogs(index,obj, channel = "default") {
      ...
      axios.post("getShopInfoTest", {  //测试环境调用接口    走文件
        centre_longitude: this.l,
        centre_latitude: this.s,
        longitude: window.LONGITUDE, // 经度
        latitude: window.LATITUDE, // 维度
